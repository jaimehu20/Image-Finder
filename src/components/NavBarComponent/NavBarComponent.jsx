...
export const NavBar = ({ setInputData }) => {
    ...
    let [burguerEnabled, setBurguerEnabled] = useState(true);
    const [closeEnabled, setCloseEnabled] = useState(true);
    const [menuEnabled, setMenuEnabled] = useState(true);

    const menuOpener = () => {
        if (burguerEnabled) {
            setBurguerEnabled(!burguerEnabled);
            burguerEnabled = false
    ...
    return <>
        <nav className="navBar">
            ...
            <h1 className="navBarTitle">IMAGE FINDER</h1>
            <input onChange={handleInputChange}className="navBarSearchBar" type="text" placeholder="Search through our image library..." />
            <svg className='navBarSearchBarIcon' width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <svg  id="ShowMenuIcon" className={burguerEnabled ? 'navBarBurguer' : 'navBarBurguerDisabled'} onClick={menuOpener} width="20" height="13" viewBox="0 0 20 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect y="10.4452" width="20" height="2" fill="black"/>
                <rect y="5.44518" width="20" height="2" fill="black"/>
                <rect y="0.445183" width="20" height="2" fill="black"/>
            </svg>
            <svg className={closeEnabled ? 'navBarCloseIcon' : 'navBarCloseIconEnabled'} onClick={menuCloser} width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1.49128 12.5013L12.4939 1.49872M12.4939 12.5013L1.49128 1.49872" stroke="#000000" strokeWidth="2"/>
            </svg>
        ...
        <div className={menuEnabled ? 'menuMobileOptions' : 'menuMobileOptionsEnabled'}>
        <p className='menuMobileOptionsText'>
            <a href="/" className='menuMobileOptionTextItem'>Explore</a>
        </p>
        <p className='menuMobileOptionsText'>
            <a href="/MyGallery" className='menuMobileOptionTextItem'>My Gallery</a>
        </p>
        </div>
    </>
}